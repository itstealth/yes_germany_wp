...
STG_NEWEST="$(ssh_stg_q "cd '${STG_DIR}' && set -a && . ./.env && set +a && \
  ...
  \"SELECT COALESCE(MAX(post_modified_gmt),'-') FROM ${PREFIX}posts WHERE post_type NOT IN ('revision');\" 2>/dev/null" \
  | tr -d '\r' | grep -E '^[0-9]{4}-|^-' | head -1)"
PROD_NEWEST="$(ssh_prod_q "cd '${PROD_ROOT}' && wp db query \"SELECT COALESCE(MAX(post_modified_gmt),'-') FROM ${PREFIX}posts WHERE post_type NOT IN ('revision');\" --skip-column-names 2>/dev/null" \
  | tr -d '\r' | grep -E '^[0-9]{4}-|^-' | head -1)"
